// ⚠ `server/src/modules/assignments/validators/assignments.validator.js`
// DAN AYNAN KO'CHIRILGAN.
import { z } from 'zod';

const objectId = z.string().regex(/^[0-9a-fA-F]{24}$/, "Noto'g'ri ID");

// Guruhlar ro'yxati. `multipart/form-data` da massiv ikki xil kelishi
// mumkin: bir nechta "groupIds" maydoni (massiv) yoki bitta vergulli
// satr. Ikkalasini ham qabul qilamiz — aks holda formaning ishlashi
// brauzer `FormData` xulqiga bog'liq bo'lib qolardi.
const groupIdList = z
...
export const createSchema = z.object({
  body: z.object({
    ...
    // multipart'da bo'sh maydon "" bo'lib keladi — uni null deb qabul
    // qilamiz, aks holda `Date("")` = Invalid Date bo'lib validatsiya
    // yiqilardi.
    dueDate: z
