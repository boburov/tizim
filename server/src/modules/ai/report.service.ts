...
import { yesterdayWindow, lastWeekWindow, lastMonthWindow, localDayKey } from './signals/pulse.signal.js';
import { fmtMoney } from "./insight-writer.service.js";

/** DAVRIY HISOBOTLAR — `services/report.service.js` ning ko'chirmasi. */
const MONTH_NAMES = MONTH_NAMES_UZ;

const pct: any = (v: any) => (v == null ? null : Math.round(v * 100));
...
const arrow = (d: any) => (d == null ? "" : d > 0 ? `+${d}%` : `${d}%`);

const metric = (key: any,label: any,value: any,unit: any,d: any = null,hint: any = "") => ({
  key,
  label,
  value,
  unit,
...
export const periodMeta = (period: any,now: any = new Date()) => {
  if (period === "daily") {
  ...
  if (period === "weekly") {
    ...
    const thursday: any = new Date(w.start.getTime() + 3 * 24 * 60 * 60 * 1000);
    const yearStart: any = new Date(Date.UTC(thursday.getUTCFullYear(), 0, 1));
...
/** DAVOMAT bo'limi. */
const attendanceSection = (pulse: any,prior: any) => {
...
const studentsSection = (pulse: any,prior: any) => {
...
/** LIDLAR bo'limi. */
const leadsSection = (pulse: any,prior: any) => {
...
const teachersSection = (pulse: any,prior: any) => {
...
export class ReportService {
  private readonly logger = new Logger('AiReport');
  ...
  private async coursesSection(branchId: any,now: any) {
  const groups = await this.groups.loadGroups(branchId);
  if (!groups.length) return null;
  const { signals } = await this.courses.collectCourseSignals(groups, now);
   ...
   */
  private async forecastSection(branchId: any,now: any) {
  const [forecast, overdue] = await Promise.all([
    this.finance.revenueForecast(branchId, now),
   ...
   */
  private async outcomeSection({ start, end }: any) {
  ...
  for (const r of rows) {
    ...
    if (r.stance === "opportunity") snap.opportunities += c;
      ...
      snap.impactAtRisk += Number(r._sum.expectedImpactAmount) || 0;
    }
  }
  return snap;
}
   ...
   */
  async buildReport(branchId: any,period: any,now: any = new Date()) {
  const meta = periodMeta(period, now);
  const prior = priorWindow(meta);

  const [pulse, priorPulse, snapshot] = await Promise.all([
    this.pulse.periodPulse({ start: meta.start, end: meta.end }),
    this.pulse.periodPulse(prior),
  ...
  if (period !== "daily") {
    const courses = await this.coursesSection(branchId, now);
  ...
  if (period === "monthly") {
    const [forecast, outcomes] = await Promise.all([
      this.forecastSection(branchId, now),
      this.outcomeSection({ start: meta.start, end: meta.end }),
    ]);
  ...
  const saved = await this.prisma.aiReport.upsert({
    where: {
  ...
  async buildReportsForAll(period: any,now: any = new Date()) {
  ...
  for (const b of branches) {
    try {
      const report = await runWithBranchContext(
        ...
        () => this.buildReport(b.id, period, now),
      );
      results.push({ branchId: String(b.id), name: b.name, periodKey: report.periodKey });
    } catch (err) {
      this.logger.error({ err, branch: b.name, period }, "AI hisobot tuzilmadi");
  ...
  const [items, total] = await Promise.all([
    ...
    this.prisma.aiReport.count({ where: filter }),
  ]);
  ...
  const doc = await this.prisma.aiReport.findFirst({
  ...
  async latestReport(period = "daily") {
  const doc = await this.prisma.aiReport.findFirst({
    where: { ...branchFilter(), period } as never,
    orderBy: { periodStart: "desc" },
  });
