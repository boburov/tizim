// ⚠ `server/src/utils/money.js` DAN AYNAN KO'CHIRILGAN.
...
export const dec = (v: unknown): Prisma.Decimal => {
  if (v instanceof D) return v as Prisma.Decimal;
...
export const num = (v: unknown): number =>
  v === null || v === undefined ? 0 : Number(dec(v));

/** Yig'indi — istalgan uzunlikdagi ro'yxat uchun. */
export const sum = (list: unknown[] = []): Prisma.Decimal =>
  list.reduce((acc: Prisma.Decimal, v) => acc.plus(dec(v)), new D(0));

export const add = (a: unknown, b: unknown): Prisma.Decimal => dec(a).plus(dec(b));
export const sub = (a: unknown, b: unknown): Prisma.Decimal => dec(a).minus(dec(b));
export const mul = (a: unknown, b: unknown): Prisma.Decimal => dec(a).times(dec(b));

/** Bo'lish — nolga bo'lish 0 qaytaradi (hisobotda ∞/NaN chiqmasin). */
export const div = (a: unknown, b: unknown): Prisma.Decimal => {
  const d = dec(b);
 ...
 */
export const round = (v: unknown): Prisma.Decimal =>
  dec(v).toDecimalPlaces(0, D.ROUND_HALF_UP);

/** Butun so'm sifatida son qaytaradi — bazaga yozish uchun. */
export const soum = (v: unknown): number => Number(round(v));

/** a ning b dan foizi (b=0 → null, "0% o'sish" yolg'on bo'lmasin). */
export const pctOf = (a: unknown, b: unknown): number | null => {
  const d = dec(b);
...
export const applyPercent = (amount: unknown, rate: unknown): Prisma.Decimal =>
  div(mul(amount, rate), 100);
 ...
 */
export const allocate = (total: unknown, weights: number[] = []): number[] => {
  const t = round(total);
  const w: Prisma.Decimal[] = weights.map((x) => dec(x));
  const totalWeight = w.reduce((a, b) => a.plus(b), new D(0));
  if (totalWeight.isZero() || !w.length) return weights.map(() => 0);

  const shares = w.map((x) =>
    Number(div(mul(t, x), totalWeight).toDecimalPlaces(0, D.ROUND_DOWN)));
...
export const isZero = (v: unknown): boolean => dec(v).isZero();
export const gt = (a: unknown, b: unknown): boolean => dec(a).greaterThan(dec(b));
export const gte = (a: unknown, b: unknown): boolean =>
  dec(a).greaterThanOrEqualTo(dec(b));
export const lt = (a: unknown, b: unknown): boolean => dec(a).lessThan(dec(b));
export const max0 = (v: unknown): Prisma.Decimal =>
  dec(v).isNegative() ? new D(0) : dec(v);
