...
| B24 | `GET /admin-dashboard/retention`, `GET /admin-dashboard/churned-students` | **FILIAL KO'LAMI UMUMAN YO'Q.** `retention.service.js` da `branchFilter`/`branchGroupFilter`/`userBranchCondition` **0 marta** ishlatiladi (qo'shni `adminDashboard.service.js` da 18 marta, `studentStats.service.js` da esa ochiq izoh bilan TUZATILGAN). Ya'ni filial direktori BUTUN TASHKILOTNING chiqib ketgan o'quvchilarini — ism, familiya, **login**, guruh va o'qituvchisi bilan — ko'radi. | ⚠⚠ HAQIQIY SIZISH, Express'da MAVJUD. Ko'chirishda **ATAYLAB TUZATILMADI**: filtr qo'shilsa NestJS Express'dan kam natija qaytarib, paritet ataylab buzilardi va xato "ko'chirish regressiyasi" bo'lib ko'rinardi. `test/admin-dashboard-parity.test.mjs` buni "ko'lam ishlayapti" deb EMAS, "MA'LUM SIZISH" deb belgilaydi va `overview` bilan yonma-yon qo'yadi (u ko'lamni QO'LLAYDI: 4 < 44). **TUZATISH ALOHIDA QAROR** — u xatti-harakatni o'zgartiradi va klient raqamlarini kamaytiradi. |
| B25 | UMUMIY TEZLIK CHEGARASI (butun API) | Express `app.js:50` da `generalLimiter` ni **GLOBAL** qo'llaydi (200 so'rov / 60s, oshsa **429**). NestJS'da `common/middleware/rate-limit.ts` da `generalLimiter` **E'LON QILINGAN, LEKIN HECH QAYERDA ULANMAGAN** — faqat `authLimiter` (auth modulida) va `botVerifyLimiter` ishlatiladi. | ⚠ HAQIQIY PARITET FARQI VA XAVFSIZLIK KAMCHILIGI: NestJS'da umumiy so'rov chegarasi YO'Q (DoS yuzasi). Ko'p so'rovli paritet yurishida Express 429, NestJS esa haqiqiy javob qaytaradi va solishtiruv "farq bor" deb yiqiladi — aslida hech narsa o'lchanmagan bo'ladi. **Bu yerda TUZATILMADI**: `generalLimiter` ni ulash BUTUN ilovaga (barcha agentlar modullariga) ta'sir qiladi va ularning testlarini 429 ga uchratardi — bu ilova bootstrap'i egasining qarori. Testlar 429 da kutib qayta uriniadi, ya'ni chegara YASHIRILMAYDI. **CUTOVER'DAN OLDIN YOPILISHI SHART.** |
...
| B20 | ⚠️ **JIDDIY** — `teacherSalary.applyPaidDelta()` TRANZAKSIYADAN CHIQIB KETADI | `salaryTransaction.writeSalaryTransaction()` uni `runFinanceTxn` ICHIDA `{ capToRemaining: true, tx }` bilan chaqiradi, LEKIN `applyPaidDelta` imzosi faqat `capToRemaining` ni oladi — `tx` E'TIBORSIZ QOLADI va xom SQL GLOBAL klientda bajariladi. `financeTxn.helper.js` ning O'Z izohi aynan shundan ogohlantiradi ("work ichida global prisma ishlatilsa rollback unga ta'sir qilmaydi"). **Tekshirib ko'rildi**: tranzaksiya ataylab bekor qilinganda `paidAmount` 0 → 50000 bo'lib QOLDI. | ⚠️ **PUL XAVFSIZLIGI XATOSI.** Oqibati: `salaryTransaction.create` yoki `postTeacherPayroll` yiqilsa, to'lov qatori va jurnal yozuvi ROLLBACK bo'ladi, `paidAmount` esa O'SGANICHA qoladi — maosh "to'langan" ko'rinadi, pul yozuvi YO'Q. Yuqoridagi "atomik: balans + tranzaksiya + jurnal BITTA amalda" izohi AMALDA NOTO'G'RI. NestJS imzoni AYNAN takrorlaydi (`tx` qabul qilinadi, ishlatilmaydi) — aks holda paritet YOLG'ON bo'lardi. **TUZATISH ALOHIDA QAROR va IKKALA stekda BIR VAQTDA qilinishi kerak.** |
| B21 | `salaryTransaction.remove()` jurnalni STORNO QILMAYDI | To'lov bekor qilinganda `SalaryTransaction` soft-delete bo'ladi va `paidAmount` kamayadi, lekin `postTeacherPayroll` yaratgan jurnal yozuvi O'Z HOLICHA qoladi (storno yozuvi qo'shilmaydi). | Chiqim hisoboti bekor qilingan to'lovni HAM ko'rsatib turadi. Ataylab o'zgartirilmadi. |
| B22 | `TeacherSalary` da `isDeleted` ustuni YO'Q | `SalaryTransaction` da esa BOR — shuning uchun filtrlar ASSIMETRIK. `teacherSalary` so'rovlariga `isDeleted: false` qo'shish Prisma'da XATO beradi. | Ataylab; `TeacherSalary` hosila jadval (o'chirilmaydi, qayta hisoblanadi). |
