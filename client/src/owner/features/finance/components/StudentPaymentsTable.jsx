...
// To'lov bo'yicha hisob-kitob (progress, qoldiq, status rangi).
const calc = (p) => {
  ...
  const barColor =
    p.status === "paid" ? "bg-emerald-500" : p.status === "partial" ? "bg-amber-500" : "bg-rose-400";
...
const StudentPaymentsTable = ({ rows = [], isLoading }) => {
  const { openModal } = useModal();

  const PayButton = ({ payment }) => (
    <Button
      size="sm"
      variant="outline"
