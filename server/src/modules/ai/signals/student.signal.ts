...
/**
 * O'QUVCHI SIGNALLARI — `signals/student.signal.js` NING KO'CHIRMASI.
 *
 * ⚠ FORMULALAR TEGILMADI: oyna uzunligi (28 kun), tushish nisbati,
 * ketma-ket qoldirish va qarz hisobi Express bilan AYNAN bir xil.
 * O'zgargani faqat: global `prisma` → DI klienti, `branchGroupMatchStage`
 * → `BranchAccessService` metodi, top-level funksiyalar → klass metodlari.
 */
const DAY_MS = 24 * 60 * 60 * 1000;
export const WINDOW_DAYS = 28;

const utcMidnight = (d: any) => {
  const x = new Date(d);
...
export class StudentSignalService {
  ...
  async attendanceSignal(studentIds: any,windows: any) {
  const { priorStart, recentStart, end } = windows;
  const branchStage = await this.branchAccess.branchGroupMatchStage("groupId");
  ...
  async absenceStreakSignal(studentIds: any,windows: any) {
  const branchStage = await this.branchAccess.branchGroupMatchStage("groupId");
  ...
  async gradeSignal(studentIds: any,windows: any) {
  const { priorStart, recentStart, end } = windows;
  const branchStage = await this.branchAccess.branchGroupMatchStage("groupId");
  ...
  async weekdayPatternSignal(studentIds: any,windows: any) {
  const branchStage = await this.branchAccess.branchGroupMatchStage("groupId");
  const since = new Date(windows.end.getTime() - 90 * DAY_MS);
  ...
  for (const r of (rows) as any[]) {
    ...
    let dayEntry = entry.days.find((x: any) => x.dow === dow);
    if (!dayEntry) {
  ...
  for (const [sid, entry] of byStudent) {
    ...
    const eligible = entry.days.filter((d: any) => d.total >= 4);
    let worst = null;
  ...
  async debtSignal(studentIds: any,now: any) {
  ...
  const filteredRows = rows.filter((r: any) => r.expectedAmount > r.paidAmount);
  ...
  async groupChurnSignal(windows: any) {
  const branchStage = await this.branchAccess.branchGroupMatchStage("groupId");
  const since = new Date(windows.end.getTime() - 90 * DAY_MS);
  ...
  async paymentDisciplineSignal(studentIds: any,now: any) {
  ...
  for (const r of groupMap.values()) {
    ...
    const entry = byStudent.get(sid);
    const periodEnd: any = new Date(Date.UTC(r.year, r.month, 0));
  ...
  async freezeSignal(studentIds: any) {
  ...
  async collectStudentSignals(students: any,now: any = new Date()) {
  const windows = buildWindows(now);
  const ids = students.map((s: any) => String(s.id || s._id));
  ...
  const [
    ...
    this.attendanceSignal(ids, windows),
    this.absenceStreakSignal(ids, windows),
    this.gradeSignal(ids, windows),
    this.debtSignal(ids, now),
    this.groupChurnSignal(windows),
    this.freezeSignal(ids),
    this.paymentDisciplineSignal(ids, now),
    this.weekdayPatternSignal(ids, windows),
  ]);
