...
> ## ⚠️ BU STEK ARXIVLANGAN — ISHGA TUSHMAYDI (2026-08-22)
>
> **Asosiy server — `server/` (NestJS).** Bu papka ilgari `server/` edi;
> cutover'da u `server_legacy/` ga ko'chdi, NestJS esa `server/` nomini
> oldi va 5000-portni egalladi (klient manzili o'zgarmadi).
>
> ⚠ **BU STEK ENDI ISHGA TUSHA OLMAYDI:** `prisma/`, `.env` va
> `uploads/` yangi `server/` papkasiga KO'CHIRILDI. Bu ataylab — ikkala
> stek bir vaqtda ishlasa cronlar IKKI MARTA yurardi.
...
> [`server/MIGRATION-CHECKLIST.md`](../server/MIGRATION-CHECKLIST.md).
