...
const run = async () => {
  ...
  try {
    ...
    // ═══════════════════════════════════════════════════════════════════
    console.log('\n\x1b[1m2. Navbat EGALIGI (cutover\'dan keyin)\x1b[0m');

    // ═══════════════════════════════════════════════════════════════════
    // ⚠ BU BO'LIM CUTOVER'DA TESKARISIGA AYLANDI (2026-08-22).
    //
    // Kesishuv davrida to'g'ri qoida "NestJS navbatga TEGMASIN" edi:
    // Express cronlarning yagona egasi bo'lgan va ikkinchi ega bo'lsa
    // oylik generatsiya IKKI MARTA yurardi.
    //
    // Express to'xtatilgach qoida TESKARI bo'ldi: endi NestJS YAGONA ega
    // bo'lishi SHART. Eski shaklda qoldirilsa test ko'chirishning
    // TUGAGANINI jazolardi — bu sessiyada AYNI xato beshta to'plamda
    // topilgan edi.
    //
    // ⚠ HIMOYA YO'QOLMAYDI, faqat YO'NALISHI o'zgardi: ilgari "ikkinchi
    // ega paydo bo'lmasin" tekshirilardi, endi "ega BOR va U BITTA".
    // ═══════════════════════════════════════════════════════════════════
    const workersOn = String(process.env.NEST_WORKERS_ENABLED ?? 'false') === 'true';
    check(
      'NestJS navbatlarning EGASI (NEST_WORKERS_ENABLED=true)',
      workersOn,
      workersOn ? '' : "cutover'dan keyin bu YOQILGAN bo'lishi SHART",
    );

    const opts = scheduler.bossOptions();
    check('sxema Express bilan bir xil', opts.schema === 'pgboss');
    check(
      "⚠ CRON SOATI YOQILGAN (`schedule !== false`)",
      opts.schedule !== false,
      'busiz birorta cron yurmasdi va buni HECH NARSA ko\'rsatmasdi',
    );

    // ⚠ PRODUSER CHEKLOVLARI OLIB TASHLANGAN BO'LISHI SHART.
    //
    // Kesishuv davrida to'rttasi ham `false` edi va TO'G'RI edi: texnik
    // xizmatni Express bajarardi. Express ketgach ular `false` bo'lib
    // qolsa pg-boss jadvallariga HECH KIM texnik xizmat qilmasdi —
    // bajarilgan ishlar tozalanmay, `pgboss` sxemasi cheksiz o'sardi.
    // Bu JIMGINA sodir bo'ladi: xato ham, sekinlashuv ham darhol
    // ko'rinmaydi.
    check(
      "⚠ TEXNIK XIZMAT YOQILGAN (`supervise !== false`)",
      opts.supervise !== false,
      'busiz bajarilgan ishlar tozalanmay, pgboss sxemasi cheksiz o\'sardi',
    );
    check("sxema ko'chirish YOQILGAN (`migrate !== false`)", opts.migrate !== false);
    check("sxema yaratish YOQILGAN (`createSchema !== false`)", opts.createSchema !== false);

    // ⚠ IKKINCHI EGA YO'QLIGI — HIMOYANING ASOSI.
    // Express arxivlangan (`server_legacy/`), lekin kimdir uni qo'lda
    // ishga tushirsa cronlar IKKI MARTA yurardi. Buni PORT orqali
    // o'lchaymiz: Express ham 5000 da turadi, ya'ni ikkalasi bir vaqtda
    // ishlay olmaydi — lekin boshqa portda ko'tarilishi MUMKIN.
    const expressAlive = await fetch('http://127.0.0.1:5001/api/health')
      .then((r) => r.ok)
      .catch(() => false);
    check(
      'IKKINCHI stek ishlamayapti (5001 bo\'sh)',
      expressAlive === false,
      expressAlive ? "⚠ 5001 da server BOR — cronlar IKKI MARTA yurishi mumkin" : '',
    );
    ...
    if (!pgbossInstalled) {
      ...
      // ⚠ CUTOVER'DAN KEYIN TESKARI: ilgari "ulanish ishchiga
      // AYLANTIRMASIN" talab qilinardi (Express yagona ishchi edi).
      // Endi NestJS ishchi BO'LISHI shart — aks holda navbatga
      // qo'yilgan ish HECH QACHON bajarilmasdi.
      check(
        '⚠ NestJS ISHCHI sifatida ishlayapti',
