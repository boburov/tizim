...
import { TRANSFER_STATUSES } from '../../common/constants/treasury.js';
...
import { JournalService, type JournalLineInput, type TxClient } from './journal.service.js';
...
export class CashTransferService {
  ...
  private async claimTransfer(
    ...
    const row = await tx.cashTransfer.findUnique({ where: { id: String(id) } });
    return row!;
  }
  ...
  async send(
    {
      ...
      note,
      shiftId,
    }: { toBranchId: string; amount: number; note?: string; shiftId?: string },
    currentUser?: Actor | null,
  ) {
    const fromBranchId = await this.branchAccess.resolveBranchForWrite(
      currentUser,
      null,
    );
    ...
    const cash = await this.journal.accountBalance(
      fromBranchId,
      ACCOUNT_KINDS.CASH,
    );
    if (value > cash) {
    ...
    const transfer = await this.prisma.$transaction(async (tx) => {
      const row = await tx.cashTransfer.create({
        data: {
          ...
          amount: value,
          status: TRANSFER_STATUSES.IN_TRANSIT as never,
  ...
  async receive(
    transferId: string,
    { countedAmount, note }: { countedAmount?: number; note?: string },
    currentUser?: Actor | null,
  ) {
    ...
    const counted =
      countedAmount === undefined || countedAmount === null
        ? (transfer.amount as unknown as number)
    ...
    const amount = transfer.amount as unknown as number;
    const discrepancy = counted - amount;
    ...
    const aLines: JournalLineInput[] = [
      { accountKind: ACCOUNT_KINDS.TRANSIT, credit: amount },
    ];
    if (counted > 0) {
      aLines.unshift({
    ...
    const saved = await this.prisma.$transaction(async (tx) => {
      ...
      const claimed = await this.claimTransfer(txc, transferId, {
        ...
        data: {
          ...
          status:
            discrepancy === 0
              ? TRANSFER_STATUSES.RECEIVED
              : TRANSFER_STATUSES.DISPUTED,
  ...
  async cancel(
    ...
    const saved = await this.prisma.$transaction(async (tx) => {
      ...
      await this.journal.post({
        ...
        lines: [
          { accountKind: ACCOUNT_KINDS.CASH, debit: claimed.amount as unknown as number },
          {
            accountKind: ACCOUNT_KINDS.TRANSIT,
            credit: claimed.amount as unknown as number,
          },
        ],
        refModel: 'CashTransfer',
        refId: claimed.id,
  ...
  async list({
    ...
    const [items, total] = await Promise.all([
      ...
      this.prisma.cashTransfer.count({ where: where as never }),
    ]);
 ...
 * uchun shu kerak).
 */
const toLegacyShape = (row: Record<string, unknown>) => ({
  ...row,
