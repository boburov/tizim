...
/** O'QUVCHI INSIGHT'LARI — `services/studentInsight.service.js` ning ko'chirmasi. */
const DAY_MS = 24 * 60 * 60 * 1000;

const buildSourceRefs = (sid: any,signals: any,kinds: any) => {
  const refs = [];
...
const detectImproving = ({ subjectLabel, signals, thresholds }: any) => {
...
const detectAttendancePattern = ({ sid, subjectLabel, signals, thresholds }: any) => {
...
export class StudentInsightService {
  ...
  private async loadMonthlyValue(studentIds: any,now: any) {
  ...
  async recomputeStudentInsights(branchId: any,now: any = new Date()) {
  ...
  const [signalsMap, valueMap] = await Promise.all([
    this.signals.collectStudentSignals(students, now),
    this.loadMonthlyValue(ids, now),
  ]);
  ...
  for (const student of students) {
    ...
    if (churn.confidence < config.confidenceFloor) {
      ...
      const res = await this.writer.upsertInsight(
        buildInsight({
          ...base,
    ...
    if (improving) {
      if (improving.confidence < config.confidenceFloor) {
        ...
        const res = await this.writer.upsertInsight(buildInsight({ ...base, ...improving }));
    ...
    if (pattern) {
      if (pattern.confidence < config.confidenceFloor) {
        ...
        const res = await this.writer.upsertInsight(buildInsight({ ...base, ...pattern }));
        stats.attendancePattern[res] += 1;
      }
    }
  }

  stats.churn.closed = await this.writer.closeStale(
    branchId,
    ["student_churn_risk"],
    stillOpen.student_churn_risk,
    now,
  );
  stats.payment.closed = await this.writer.closeStale(
    branchId,
    ["payment_risk"],
    stillOpen.payment_risk,
    now,
  );
  stats.improving.closed = await this.writer.closeStale(
    branchId,
    ["student_improving"],
    stillOpen.student_improving,
    now,
  );
  stats.attendancePattern.closed = await this.writer.closeStale(
    branchId,
    ["attendance_anomaly"],
