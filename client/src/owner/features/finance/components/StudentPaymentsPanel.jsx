...
const StudentPaymentsPanel = () => {
  ...
  const hasActiveFilters = filters.groupId || debouncedSearch;

  const resetFilters = () => {
    filters.setFields({ groupId: "", search: "" });
    setPage(1);
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-3 rounded-lg border bg-card p-3 sm:grid-cols-2 lg:grid-cols-3">
        <SelectField
          searchable
          label="Guruh"
      ...
      {isError ? (
        ...
        <>
          ...
          <div className={`transition-opacity ${isFetching ? "opacity-60" : ""}`}>
            <StudentPaymentsTable rows={payments} />
          </div>
