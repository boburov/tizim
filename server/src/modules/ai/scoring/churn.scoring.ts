 ...
 */
const squash = (x: any,k: any = 6,x0: any = 0.45) => 1 / (1 + Math.exp(-k * (x - x0)));

const readMap = (map: any,defaults: any) => {
 ...
 */
export const computeConfidence = ({ lessons, enrolledDays, factors, lastSignalDays }: any) => {
  ...
  const active = factors.filter((f: any) => f.normalized > 0.05);
  let consistency = 1;
  if (active.length >= 2) {
    const mean = active.reduce((a: any,f: any) => a + f.normalized, 0) / active.length;
    const variance =
      active.reduce((a: any,f: any) => a + (f.normalized - mean) ** 2, 0) / active.length;
 ...
 */
export const scoreChurn = (signals: any,config: any = {}) => {
 ...
 */
export const churnActions = (factors: any,{ debtAmount = 0 }: any = {}) => {
  const actions = [];
  const by = Object.fromEntries(factors.map((f: any) => [f.key, f]));
