...
/** KURS SIGNALLARI — `signals/course.signal.js` ning ko'chirmasi. */
...
export class CourseSignalService {
  ...
  async groupsByCourse(groups: any) {
  const courseIds: any[] = [
    ...new Set(groups.filter((g: any) => g.courseId).map((g: any) => String(g.courseId))),
  ];
  ...
  async courseAttendance(buckets: any,now: any) {
  const windows = buildWindows(now);
  const allGroupIds: string[] = buckets.flatMap((b: any) => b.groups.map((g: any) => String(g.id || g._id)));
  ...
  async courseEnrollment(buckets: any,now: any) {
  const allGroupIds: string[] = buckets.flatMap((b: any) => b.groups.map((g: any) => String(g.id || g._id)));
  if (!allGroupIds.length) return new Map();
  ...
  async courseRevenue(buckets: any,now: any) {
  const allGroupIds: string[] = buckets.flatMap((b: any) => b.groups.map((g: any) => String(g.id || g._id)));
  if (!allGroupIds.length) return new Map();
  ...
  async collectCourseSignals(groups: any,now: any = new Date()) {
  const buckets = await this.groupsByCourse(groups);
  if (!buckets.length) return { buckets: [], signals: new Map() };

  const [attendance, enrollment, revenue] = await Promise.all([
    this.courseAttendance(buckets, now),
    this.courseEnrollment(buckets, now),
    this.courseRevenue(buckets, now),
  ]);
