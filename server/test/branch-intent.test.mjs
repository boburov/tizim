/**
 * ═══════════════════════════════════════════════════════════════════════════
 * FILIAL NIYATI TASDIG'I — `server_legacy/tests/branchIntent.test.js` dan
 * ko'chirilgan.
 *
 * ── NEGA KO'CHIRILDI (dalil) ──
 * `src/common/rbac/branch-intent.ts` NestJS'ga ko'chirilgan va
 * `middleware/auth.middleware.ts:113` da HAR BIR so'rovda chaqiriladi,
 * lekin NestJS tomonida uni sinaydigan BIRORTA test yo'q edi —
 * `grep -rl "branch-intent" test/` BO'SH qaytardi. Express tomonida esa
 * unga atalgan 136 qatorli to'plam bor edi.
 *
 * ── SAVOL ──
 * "Client 'A filialga yozyapman' deb o'ylab turganda, server uni jimgina
 * B filialga yozib qo'ymasligiga ishonchim komilmi?"
 *
...
} from '../dist/common/rbac/branch-intent.js';
...
console.log("\n\x1b[1mFILIAL NIYATI TASDIG'I (NestJS)\x1b[0m");
