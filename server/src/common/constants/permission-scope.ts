...
//
// `server_legacy/src/constants/permissionScope.js` dan ko'chirilgan.
//
// ⚠ NEGA U KO'CHIRISHDA TUSHIB QOLGAN EDI: bu faylni ish vaqtida HECH KIM
// import qilmaydi — uning yagona iste'molchisi `permissions.seed.js` va
// `migrateDirectorFullAccess.seed.js`. Marshrut/modul bo'yicha yurgan
// ko'chirish uni ko'rmagan. Natijada NestJS tomonida direktor rolini
// yaratadigan mantiq umuman yo'q edi.
//
...
export const OWNER_ONLY_PERMISSIONS: readonly string[] = Object.freeze([
  ...
  // Owner-ga tenglashtiradi: `RolesGuard` shu kalitni ham qabul qiladi
  // (`common/guards/roles.guard.ts`), ya'ni butun owner-only bo'limni
  // ochib yuboradi.
  ...
  // (`server_legacy/tests/privEscalation.test.js` aynan shu drift'ni
  //  tutgan edi.)
  PERMISSIONS.BRANCHES_VIEW_ALL,

  // Filial ochish/tahrirlash/o'chirish. Tahrirlash ayniqsa xavfli:
  // delegatsiya matritsasi Branch yozuvida saqlanadi, ya'ni direktor
  ...
  // qo'ymaydi (`modules/roles/roles.helper.ts`).
...
const ownerOnlySet = new Set<string>(OWNER_ONLY_PERMISSIONS);

/** Ruxsat faqat owner'gami. */
export const isOwnerOnlyPermission = (key: string): boolean =>
  ownerOnlySet.has(key);
 ...
 */
export const BRANCH_LOCAL_PERMISSIONS: readonly string[] = Object.freeze(
