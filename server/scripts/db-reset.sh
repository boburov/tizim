#!/usr/bin/env bash
#
# BAZANI TOZALASH + BOOTSTRAP SEED — bitta buyruq.
#
#   npm run db:reset                  # so'raydi, keyin tozalaydi va seed yurgizadi
#   npm run db:reset -- -y            # so'ramaydi (CI uchun)
#
# ⚠ `server_legacy/scripts/db-reset.sh` DAN KO'CHIRILGAN (2026-08-25).
# Express stek o'chirildi; farqi — seed'lar endi NestJS `dist/` dan
# yurgiziladi va ro'yxat TO'LIQROQ (Express versiyasi faqat permissions
# va owner ni yurgizardi, aloqa standartlari bilan chiqim turkumlarini
# TASHLAB KETARDI).
#
...
# tanlab tozalashni qo'llab-quvvatlamaydi.
SKIP_PROMPT=0
for arg in "$@"; do
  case "$arg" in
    -y|--yes) SKIP_PROMPT=1 ;;
...
if [ "$SKIP_PROMPT" -eq 0 ]; then
  ...
  echo "  «${DB_NAME}» bazasi tozalanadi va bootstrap seed qaytadan yuriladi."
...
# --- 3-qadam: tozalash + seed ---
# `prisma migrate reset` schema'ni butunlay qayta quradi va BARCHA
# migratsiyalarni qaytadan qo'llaydi, ya'ni `gen_object_id()` funksiyasi
# va qisman unique indekslar ham tiklanadi - ularni qo'lda yaratish
# shart emas (unutib qo'yish xavfi yo'q).
echo "══════ 3/3  Tozalash ══════"
npx prisma migrate reset --force --skip-seed --skip-generate

# BOOTSTRAP SEED - MAJBURIY QADAM.
#
# Tozalashdan keyin `roles`/`permissions` BO'SH qoladi. Owner buni
# sezmaydi (uning ruxsati kodda ["*"] bilan qattiq bog'langan), lekin
# qolgan HAMMA rol yo'qoladi: direktor/administrator/o'qituvchi yarata
# olmaysiz, mavjudlari esa ruxsatsiz qoladi.
#
# ⚠ `seed:bootstrap` AVVAL `npm run build` ni yurgizadi: seed'lar
# `dist/` dan ishlaydi va eski emit bilan yurgizish JIMGINA noto'g'ri
# katalog yozardi.
echo "══════ Bootstrap seed (ruxsat, rol, owner, aloqa, chiqim turkumlari) ══════"
