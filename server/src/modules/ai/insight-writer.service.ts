...
/**
 * INSIGHT YOZUVCHI — `services/insightWriter.service.js` ning KO'CHIRMASI.
 *
 * ⚠ `fmtMoney`, `computePriority`, `buildInsight`, `mkStats` MODUL
 * DARAJASIDA qoldi: ular sof (bazasiz) va boshqa modullardan oddiy
 * import bilan olinadi — DI grafigiga qo'shilishi shart emas.
 */
 ...
 */
export const computePriority = ({ amount, score, severity, confidence, kind }: any) => {
...
@Injectable()
export class InsightWriterService {
  constructor(
  ...
  if (existing) {
    ...
    await this.prisma.insight.update({ where: { id: existing.id }, data: doc });
    return "updated";
  }
  await this.prisma.insight.create({ data: doc });
  return "created";
}
   ...
   */
  async closeStale(branchId: any,kinds: any,stillOpen: any,now: any = new Date()) {
  const kindList = Array.isArray(kinds) ? kinds : [kinds];
  const closable = await this.prisma.insight.findMany({
    where: {
      branchId: toId(branchId),
      kind: { in: kindList },
      status: { in: OPEN_STATUSES as never },
  ...
  const res = await this.upsertInsight(candidate);
  stats[res] += 1;
