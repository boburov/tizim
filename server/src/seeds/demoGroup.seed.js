...
const seed = async () => {
  ...
  // Servis qatlami ba'zi yo'llarda `_id` taxallusi bilan qaytaradi
  // (utils/serialize.js), ba'zilarida xom Prisma obyektini - shuning uchun
  // kalit BIR MARTA hal qilinadi.
  ...
  for (let i = 0; i < STUDENT_COUNT; i += 1) {
    const s = await prisma.user.create({ data: {
  ...
  for (const s of students) {
    await paymentService.recalcForStudent(s.id);
  }
  ...
  logger.info(`   Guruh:     ${group.name}  (id: ${groupId})`);
