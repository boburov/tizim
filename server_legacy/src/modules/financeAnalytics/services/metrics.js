import { num, pctOf, div, mul, sub } from "../../../utils/money.js";
 ...
 * @returns {{current:number, previous:number, change:number, changePercent:number|null}}
 */
export const compare = (current, previous) => {
  const c = num(current);
 ...
 */
export const ratioPercent = (part, whole) => pctOf(part, whole);

/** Bo'lish — maxraj nol bo'lsa `null` (masalan o'quvchi boshiga daromad). */
export const per = (total, count) => {
...
export const applyRate = (amount, ratePercent) =>
 ...
 */
export const rankBy = (rows, keys, limit = null) => {
  const out = {};
