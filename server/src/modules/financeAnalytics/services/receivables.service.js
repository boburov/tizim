 ...
 * oyning 5-sanasi"), o'zgartiriladigan yagona joy — shu yerdagi
 * `DUE_DATE` ifodasi.
 *
 * ── HISOBDAN CHIQARILGAN QARZ (write-off) ──
 * `writtenOff` qatorlar kutilgan/to'langan/qoldiqdan CHIQARILADI va
 * alohida `badDebt` bo'lib beriladi — mavjud `financeReport.service.js`
 * bilan AYNAN bir xil qoida (ikki hisobot bir xil raqam berishi shart).
 */

// Oyning oxirgi kuni — to'lov muddati.
const DUE_DATE = Prisma.raw(
  `(make_date(sp.year, sp.month, 1) + INTERVAL '1 month - 1 day')`,
);
...
const planWhere = ({ from, to, branchId, dimensions = {} }) => {
  const parts = [
    // Plan OYI tanlangan oraliqqa tushadimi (indeksdan foydalanadigan
    // shakl — qarang analyticsFilter.js → planPeriodClause).
    planPeriodClause("sp", from, to),
  ];
  const bc = branchClause('sp."branchId"', branchId);
  if (bc !== Prisma.empty) parts.push(bc);
  if (dimensions.groupId) parts.push(Prisma.sql`AND sp."groupId" = ${String(dimensions.groupId)}`);
  if (dimensions.studentId) parts.push(Prisma.sql`AND sp."studentId" = ${String(dimensions.studentId)}`);
  if (dimensions.courseId) parts.push(Prisma.sql`AND g."courseId" = ${String(dimensions.courseId)}`);
...
export const getReceivables = async (filters = {}) => {
  ...
  const where = planWhere({ ...range, branchId: filters.branchId || null, dimensions: filters });
...
const GROUPINGS = Object.freeze({
  branch: { col: 'sp."branchId"', join: Prisma.empty, label: "branch" },
  group: { col: 'sp."groupId"', join: Prisma.empty, label: "group" },
  course: { col: 'g."courseId"', join: Prisma.empty, label: "course" },
  student: { col: 'sp."studentId"', join: Prisma.empty, label: "student" },
});

export const getReceivablesBy = async (by, filters = {}) => {
  ...
  const names = await resolveNames(by, ids);
...
/** Nomlarni BITTA so'rov bilan yechadi (N+1 emas). */
const resolveNames = async (by, ids) => {
