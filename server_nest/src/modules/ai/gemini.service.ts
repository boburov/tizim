...
/**
 * GEMINI CHAQIRUVI — `services/gemini.service.js` ning KO'CHIRMASI.
 *
 * ⚠ `env.GEMINI_MODEL` standart PARAMETR qiymati edi; Nest'da u
 * misol maydoni (`this.env`) bo'lgani uchun standart qiymat tana
 * ichiga ko'chdi (`model = model || this.env.GEMINI_MODEL`). Natija
 * AYNAN bir xil.
 */
 ...
 */
const buildPrompt = (insight: any) => {
  const factors = (insight.factors || [])
    .filter((f: any) => f.normalized > 0.05)
    .slice(0, 5)
    .map((f: any) => `- ${f.label}: ${f.value}${f.unit ? " " + f.unit : ""}`)
    .join("\n");
...
const buildFinancePrompt = (signal: any) => {
...
export class GeminiService {
  ...
  private readonly env: { GEMINI_API_KEY: string; GEMINI_MODEL: string };
  ...
  const seed = JSON.stringify({
    ...
    factors: (insight.factors || [])
      .filter((f: any) => f.normalized > 0.05)
      .map((f: any) => [f.key, Math.round(f.value)]),
   ...
   */
  async generateNarration(insight: any,model: any = null) {
  model = model || this.env.GEMINI_MODEL;
  if (!this.isNarrationConfigured()) return null;
  ...
  const log = (ok: any,errorCode: any,usage: any = {}) =>
  ...
  try {
    const res = await fetch(
      `${API_BASE}/${model}:generateContent?key=${this.env.GEMINI_API_KEY}`,
      {
        method: "POST",
    ...
    if (!res.ok) {
      ...
      this.logger[level](`Gemini narrator javob bermadi (status ${res.status})`);
    ...
    if (finishReason && finishReason !== "STOP") {
      this.logger.debug({ finishReason }, "Gemini javobi to'liq tugamadi");
    ...
    if (clean.length < MIN_NARRATION_LENGTH || clean.length > MAX_NARRATION_LENGTH) {
      this.logger.debug({ length: clean.length }, "Gemini izohi rad etildi (uzunlik)");
    ...
    if (!aborted) {
      this.logger.warn(`Gemini narrator xatosi: ${err.message}`);
    }
   ...
   */
  async generateFinanceExplanation(signal: any,model: any = null) {
  model = model || this.env.GEMINI_MODEL;
  if (!this.isNarrationConfigured()) return null;
  ...
  try {
    ...
    if (!res.ok) {
      const level = res.status === 429 ? "debug" : "warn";
      this.logger[level](`Gemini moliyaviy izoh bermadi (status ${res.status})`);
      await log(false, String(res.status));
      return null;
    }
    ...
    const err = e as Error;
    this.logger.warn(`Gemini moliyaviy izoh xatosi: ${err?.message}`);
