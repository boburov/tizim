...
import { yesterdayWindow } from './signals/pulse.signal.js';
import { shiftMonth, monthKey } from './signals/finance.signal.js';
import { monthNameUz } from "../../common/constants/calendar.js";
import { fmtMoney } from "./insight-writer.service.js";

/** BRIFING — `services/briefing.service.js` ning ko'chirmasi. */
 ...
 */
const buildKpis = ({ raw, forecast, attendance }: any) => {
 ...
 */
const buildSummary = ({ counts, risks, opportunities, health, today, overdue }: any) => {
...
export class BriefingService {
  constructor(
    ...
    private readonly health: HealthSignalService,
    private readonly recompute: RecomputeService,
  ) {}

  /** 1. KECHA NIMA BO'LDI - o'lchangan fakt, oldingi kun bilan taqqoslangan. */
  private async buildYesterday(now: any) {
  const y: any = yesterdayWindow(now);
  const dayBefore = {
  ...
  const [pulse, prior] = await Promise.all([
    this.pulse.periodPulse(y),
    this.pulse.periodPulse(dayBefore),
  ]);
  ...
  private async buildToday(now: any) {
  const snap = await this.pulse.todaySnapshot(now);
  ...
  const overdue = snap.followUps.filter((f: any) => f.overdue).length;
  if (overdue) {
   ...
   */
  private async buildNext(branchId: any,now: any) {
   ...
   */
  private async buildForecastBlock({ now, forecast, attendance }: any) {
  const year = now.getUTCFullYear();
  const month = now.getUTCMonth() + 1;

  const collected = await this.finance.collectedByMonth(7, now);
  const byKey = new Map(collected.map((r: any) => [r.key, r]));
  ...
  const history = [5, 4, 3, 2, 1]
    ...
    .map((p) => {
      ...
      return {
        ...
        amount: (row as any)?.amount || 0,
        transactions: (row as any)?.count || 0,
        isForecast: false,
      };
    });
   ...
   */
  async buildBriefing({ now = new Date(), actionLimit = 6 } = {}) {
  ...
  const [yesterday, today, now4, lastRun, health, attendance] = await Promise.all([
    this.buildYesterday(now),
    this.buildToday(now),
    this.buildNow(actionLimit),
    ...
    this.health.businessHealth(now),
    this.pulse.attendanceOutlook(now),
  ]);

  const next = branchId ? await this.buildNext(branchId, now) : null;
  ...
  const forecast = await this.buildForecastBlock({
    now,
