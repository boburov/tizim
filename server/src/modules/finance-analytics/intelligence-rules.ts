 ...
 * (`services/intelligenceRules.js` NING KO'CHIRMASI)
 * ══════════════════════════════════════════════════════════════════════
 *
 * ── ENG MUHIM CHEGARA ──
 ...
 *
 * ⚠ SERVIS EMAS, SOF FUNKSIYALAR: bu fayl bazaga ham, DI ga ham
 * murojaat qilmaydi. `@Injectable` ga o'ralganda qoidalar test
 * qilinishi uchun butun Nest konteyneri ko'tarilishi kerak bo'lardi —
 * holbuki ular oddiy kirish→chiqish funksiyalari.
 *
 * ⚠ `Ctx` ATAYLAB `any`: kirish — o'nlab tahlil servisining natijasi
 * va ularning har biri o'z shakliga ega. Qat'iy tur yozilsa u
 * hisobotlar shakli bilan birga eskirardi va TS xatosi xatti-harakat
 * xatosidan CHALG'ITARDI. Qoidalar `?.` bilan himoyalangan, ya'ni
 * yo'q maydon signal chiqarmaydi — yiqilmaydi.
 */

/** Tahlil natijalari to'plami — `financial-intelligence.service.ts` yig'adi. */
type Ctx = any;
...
const ev = (
  label: string,
  {
    ...
    changePercent = null,
    unit = "so'm",
  }: { current: unknown; previous?: unknown; changePercent?: unknown; unit?: string },
) => ({
...
export const ruleExpenseGrowth = ({ expenses, summary }: Ctx): Ctx[] => {
  const out: Ctx[] = [];
...
export const ruleCollection = ({ summary, receivables }: Ctx): Ctx[] => {
...
/** 3 — ESKI QARZ (60+ kun). */
export const ruleReceivableAging = ({ receivables }: Ctx): Ctx[] => {
...
/** 4 — BYUDJETDAN OSHISH. */
export const ruleBudgetOverspend = ({ budget }: Ctx): Ctx[] => {
...
export const ruleDiscountAnomaly = ({ discounts, summary }: Ctx): Ctx[] => {
...
export const ruleRefundAnomaly = ({ refunds }: Ctx): Ctx[] => {
...
export const ruleDirectionRisk = ({ directions, directionsPrev }: Ctx): Ctx[] => {
  const prevById = new Map<string, Ctx>((directionsPrev?.items || []).map((d: Ctx) => [d.courseId, d]));
  const out: Ctx[] = [];
 ...
 */
export const ruleTeacherRisk = ({ teachers, teachersPrev }: Ctx): Ctx[] => {
  const prevById = new Map<string, Ctx>((teachersPrev?.items || []).map((t: Ctx) => [t.teacherId, t]));
  const out: Ctx[] = [];
...
/** 9 — BO'SH TURGAN XONA. */
export const ruleRoomCapacity = ({ rooms }: Ctx): Ctx[] => {
  const out: Ctx[] = [];
 ...
 * hisoblash emas.
 */
export const ruleConcentration = ({ groups }: Ctx): Ctx[] => {
  const items = (groups?.items || []).filter((g: Ctx) => g.contributionProfit > 0);
  if (items.length < THRESHOLDS.concentrationTopN + 1) return [];

  const total = items.reduce((s: number, g: Ctx) => s + g.contributionProfit, 0);
  if (total <= 0) return [];
  const top = [...items]
    .sort((a: Ctx, b: Ctx) => b.contributionProfit - a.contributionProfit)
    .slice(0, THRESHOLDS.concentrationTopN);
  const topSum = top.reduce((s: number, g: Ctx) => s + g.contributionProfit, 0);
  ...
  return [signal({
    ...
    evidence: [
      ...
      ...top.map((g: Ctx) => ev(g.name, { current: g.contributionProfit })),
    ],
...
export const rulePositive = ({ summary, directions, directionsPrev }: Ctx): Ctx[] => {
  const out: Ctx[] = [];
