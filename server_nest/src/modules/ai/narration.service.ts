/**
 * QOLIP MATNLARI — `services/narration.service.js` ning KO'CHIRMASI.
 *
 * ⚠ SOF MODUL: LLM chaqirmaydi, bazaga tegmaydi. LLM bilan boyitish
 * `narration-queue.service.ts` da, ALOHIDA turadi.
 */
const SEVERITY_LABEL: any = {
  high: "Yuqori",
  medium: "O'rta",
  low: "Past",
};
...
export const narrate = ({
  ...
  const active = factors.filter((f: any) => f.normalized > 0.05).slice(0, maxFactors);
 ...
 */
export const narrateChurn = ({ subjectLabel, score, confidence, severity, factors, expectedImpact }: any) => {
  const active = factors.filter((f: any) => f.normalized > 0.05).slice(0, 4);
...
export const narratePaymentRisk = ({ subjectLabel, score, factors, expectedImpact }: any) => {
  const active = factors.filter((f: any) => f.normalized > 0.05).slice(0, 3);
  const pct = Math.round(score * 100);
