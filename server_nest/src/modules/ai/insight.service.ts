...
/**
 * INSIGHT O'QISH VA HOLAT O'ZGARTIRISH — `services/insight.service.js`
 * ning KO'CHIRMASI.
 *
 * ⚠ FILIAL KO'LAMI: har bir so'rov `branchFilter()` orqali cheklanadi
 * va `findScoped` yozuvni ko'lamdan TASHQARIDA topsa 404 beradi —
 * Express bilan aynan bir xil.
 */
...
export class InsightService {
  ...
  const [items, total] = await Promise.all([
    ...
    this.prisma.insight.count({ where: filter }),
  ]);
  ...
  const totals = await this.prisma.insight.groupBy({
    by: ["severity", "stance"],
    where: filter as never,
    _count: { _all: true },
  ...
  const summary: any = { high: 0, medium: 0, low: 0, opportunities: 0, impactAtRisk: 0, upside: 0 };
   ...
   */
  async bySubjects(subjectIds: any = []) {
   ...
   */
  async byDomain(domain: any,query: any = {}) {
  ...
  private async findScoped(id: any) {
  const doc = await this.prisma.insight.findFirst({
  ...
  async acknowledge(id: any,user: any) {
  const doc = await this.findScoped(id);
  ...
  async resolve(id: any,user: any) {
  const doc = await this.findScoped(id);
  const data: any = { status: "done", resolvedAt: new Date() };
  ...
  return withLegacyId(
    await this.prisma.insight.update({ where: { id: doc.id }, data }),
  );
}
   ...
   */
  async dismiss(id: any,reason: any,user: any) {
