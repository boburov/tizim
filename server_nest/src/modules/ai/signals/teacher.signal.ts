...
/** O'QITUVCHI SIGNALLARI — `signals/teacher.signal.js` ning ko'chirmasi. */
...
export class TeacherSignalService {
  ...
  async loadTeachers(branchId: any) {
  ...
  async teacherAbsenceSignal(teacherIds: any,groupIds: any,now: any) {
  ...
  async teacherLoadSignal(teachers: any) {
  const allGroupIds: any[] = [...new Set(teachers.flatMap((t: any) => t.groupIds.map(String)))];
  ...
  const byGroup = new Map(rows.map((r: any) => [r.groupId, r._count.studentId]));
  ...
  async teacherOutcomeSignal(teachers: any,now: any) {
  ...
  for (const t of (teachers) as any[]) {
    ...
    for (const gid of t.groupIds) {
      const a: any = attByGroup.get(String(gid));
      if (a?.lessons) {
        lessons += a.lessons;
  ...
  async teacherPaymentSignal(teachers: any,now: any) {
  const allGroupIds: any[] = [...new Set(teachers.flatMap((t: any) => t.groupIds.map(String)))];
  if (!allGroupIds.length) return new Map();
  ...
  teacherBaseline(outcomes: any,loads: any,payments: any) {
  ...
  const mean = (arr: any) => (arr.length ? arr.reduce((a: any,b: any) => a + b, 0) / arr.length : null);
  ...
  async collectTeacherSignals(branchId: any,now: any = new Date()) {
  const { teachers, groups } = await this.loadTeachers(branchId);
  if (!teachers.length) return { teachers: [], groups, signals: new Map(), baseline: null };

  const teacherIds = teachers.map((t: any) => String(t.id || t._id));
  const groupIds = groups.map((g: any) => g.id || g._id);

  const [absence, loads, outcomes, payments] = await Promise.all([
    this.teacherAbsenceSignal(teacherIds, groupIds, now),
    this.teacherLoadSignal(teachers),
    this.teacherOutcomeSignal(teachers, now),
    this.teacherPaymentSignal(teachers, now),
  ]);

  const baseline = this.teacherBaseline(outcomes, loads, payments);
